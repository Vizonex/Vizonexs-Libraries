...
* [contributing](#Contributions) Things I am Contributing to or am a co-author or maintainer of. If anything is note-worthy I will try to point it out.
...
* [Rust](#Rust) Rust related projects (These could even be commandline tools example: __ruff__) or rust crates.
...
| [cyjs](https://github.com/Vizonex/cyjs) | Quick-js-ng bindings for cython & python with the aim of providing webscraping features (examples: Solving Captchas or cracking request keys or other cookies) & supporting ECMA6 | Owner | Active | Motivated because of pyduktape3 lacking maintainabiliy after the C backend was abandoned but also due to the GPL License being extremely restrictive |
...
# Rust
Rust is another low level coding language with pretty strict rules but they are in place to help any coder be a good developer.
It is used in many different ways including in the python bcrypt library which I bring up as a good example.



| Library/Tool | Summary | Notable Contributions or Comments | Currently trying to do |
|---------|---------|-----------------------|--------------------|
| __Citron__ (My biggest and most ambitious project of 2026) | A Cython tool inspired by [ruff](https://github.com/astral-sh/ruff) that will help with code formatting, 1. General tooling such as Generation of `.pyi` stub files, splitting `.pxi` formats into `.pxd` and `.pyx` formats (expect to see a few transformative features). 2. Provide helpful Optimization Code tips, for example: Recommending Changes to the current cython code `cdef list l l.append() to PyList_Append` which is a C Call instead. 3. Act as a newly made cython code formatter tool with simillar techniques to [ruff](https://github.com/astral-sh/ruff) and quite possibly [black](https://github.com/psf/black). 4. Providing templates for Cython and CPython or Maybe even CPython capsules? | This is an extermely ambitious project that I plan to take in rather slowly Currently the lexer is being worked on whith many things from ruff being carefully looked at so the wheel doesn't need reinventing. Already told the aio-libs maintainers that I would be working on this project on/off. Currently there have been no successful things out of this but some successful formations such as a cursor have been made. | Writing the Lexer/tokenizer for the Cython Language before moving onto the AST Which may or may not be a complete Nightmare to work on. I may first write the lexer by hand before migrating to autogenerating with python due to the numerous switch-cases I need. The reason why I want to rewrite the Cython AST is because it's old and could be easily upgraded to be faster, The plan will be to make the Lexer avalible as a rust crate for developers to publically use which could help patch future bugs if found | 
